        ...
        try:
            ...
            if page.query_selector(sucesso_selector):
                ...
                try:
                    ...
                    print(f"[AVISO] Falha ao ler o body_text para validade do FGTS: {body_ex}")
                ...
                if validade_match:
                    ...
                    if len(partes) == 3:
                        ...
                        print(f"[FGTS] Vencimento real extraído: {data_vencimento}")
                ...
                return {"status": "sucesso", "pdf_path": temp_pdf_path, "data_vencimento": data_vencimento}
            else:
...
def obter_federal(cnpj, user_data_dir=None, headless=True):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            body_text = page.locator("body").inner_text()
            if "insuficientes" in body_text or "Não foi possível concluir" in body_text:
                print("[FEDERAL] Impedimento para nova emissão detectado. Tentando obter última certidão válida (segunda via)...")
                ...
                if is_resultado_insuficientes:
                    ...
                    print("[FEDERAL] Tela de resultado 'insuficientes' detectada. Clicando em '+ Nova Consulta'...")
                    ...
                    if modal_found:
                        ...
                        if page.locator(btn_consultar_modal).is_visible():
                            ...
                            print("[FEDERAL] Clicou em Consultar Certidão no modal.")
                        else:
                            btn_text_sel = 'button:has-text("Consultar Certidão")'
                            if page.locator(btn_text_sel).is_visible():
                                page.click(btn_text_sel)
                                print("[FEDERAL] Clicou em Consultar Certidão no modal via texto.")
                            else:
                                try:
                                    page.locator("modal-container .modal-footer button").first.click()
                                    print("[FEDERAL] Clicou no primeiro botão do modal-footer.")
                                except Exception as modal_click_err:
                                    print(f"[FEDERAL] Falha ao tentar clicar no modal: {modal_click_err}")
                        time.sleep(2.0)
                    else:
                        # Sem modal - tenta "Consultar Certidão" direto no formulário
                        print("[FEDERAL] Modal não apareceu após Nova Consulta. Tentando Consultar direto...")
                        ...
                        if page.locator(btn_consultar_direto).is_visible():
                            ...
                            return {"status": "erro", "mensagem": "Não foi possível acessar a tela de consulta de certidões existentes."}
                    ...
                    try:
                        ...
                        print("[FEDERAL] Clicou no botão Consultar do formulário de consulta.")
                        time.sleep(2.0)
                    except:
                        try:
                    ...
                    print("[FEDERAL] Erro 023 do portal detectado (rate-limit). Aguardando 10s antes de consultar...")
                    time.sleep(10.0)
                    ...
                    for tentativa in range(1, 4):  # Até 3 tentativas
                        if page.locator(btn_consultar_direto).is_visible():
                            page.click(btn_consultar_direto)
                            print(f"[FEDERAL] Tentativa {tentativa}: Clicou em Consultar Certidão.")
                            time.sleep(3.0)
                            
                            # Verifica se o erro 023 desapareceu
                            body_after = page.locator("body").inner_text()
                            if "Não foi possível concluir" not in body_after:
                                print("[FEDERAL] Erro 023 resolvido! Prosseguindo com consulta...")
                                break
                            else:
                                if tentativa < 3:
                                    wait_time = 15 * tentativa
                                    print(f"[FEDERAL] Erro 023 persiste. Aguardando {wait_time}s antes da próxima tentativa...")
                                    time.sleep(wait_time)
                                else:
                                    page.screenshot(path="debug_federal_error.png")
                                    return {"status": "erro", "mensagem": "Portal da Receita Federal temporariamente indisponível (erro 023). Tente novamente em alguns minutos."}
                        else:
                            return {"status": "erro", "mensagem": "Botão Consultar Certidão não encontrado no formulário."}
                ...
                # Na tabela de resultados, localiza a linha da certidão válida com maior data de vencimento
                try:
                    page.wait_for_selector("datatable-body-row", timeout=15000)
                except Exception as table_err:
                    page.screenshot(path="debug_federal_error.png")
                    print(f"[FEDERAL] Tabela de resultados não carregou: {table_err}")
                    return {"status": "erro", "mensagem": f"Tabela de certidões não carregou após consulta: {table_err}"}
                ...
                print(f"[FEDERAL] Encontradas {len(rows)} linhas na tabela de certidões.")
                for idx, row in enumerate(rows):
                    cells_text = [c.inner_text().strip() for c in row.locator("datatable-body-cell").all()]
                    print(f"[FEDERAL] Linha {idx}: {cells_text}")
                ...
                if not valid_rows:
                    return {"status": "erro", "mensagem": "As informações são insuficientes e nenhuma certidão válida anterior foi localizada no portal."}
                ...
                if best_date < datetime.today().date():
                    return {"status": "erro", "mensagem": f"A certidão válida mais recente expirou em {best_date.strftime('%d/%m/%Y')}."}
                
                print(f"[FEDERAL] Baixando segunda via da certidão com validade até: {best_date.strftime('%d/%m/%Y')}")
            ...
            try:
                page.screenshot(path="debug_federal_error.png")
                print("[FEDERAL DEBUG] Screenshot de erro salvo em debug_federal_error.png")
            except:
                pass
            return {"status": "erro", "mensagem": str(e)}
        finally:
