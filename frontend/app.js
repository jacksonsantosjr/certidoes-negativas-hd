...
const dom = {
    ...
    screenUsuarios: document.getElementById('screen-usuarios'),
    ...
    btnNavUsuarios: document.getElementById('btn-nav-usuarios'),
    ...
    btnAddUsuario: document.getElementById('btn-add-usuario'),
    modalUsuario: document.getElementById('modal-usuario'),
    btnCloseModalUsuario: document.getElementById('btn-close-modal-usuario'),
    formUsuario: document.getElementById('form-usuario'),
    btnCancelarUsuario: document.getElementById('btn-cancelar-usuario'),
    
    heroSection: document.getElementById('hero-section'),
    btnHeroAcessar: document.getElementById('btn-abrir-login'),
    modalLogin: document.getElementById('modal-login'),
    btnCloseModalLogin: document.getElementById('btn-close-modal-login'),
    formLogin: document.getElementById('form-login-usuario'),
...
async function carregarDados() {
    ...
    try {
        ...
        const [empresasRes, certidoesRes, filaRes, usuariosRes] = await Promise.all([
...
function configurarNavegacao() {
    const screens = [
        ...
        { btn: dom.btnNavUsuarios, screen: dom.screenUsuarios, title: 'Gestão de Usuários', subtitle: 'Controle de acessos e perfis de permissão do sistema CND.' }
    ];
...
function configurarModais() {
    ...
    // Abertura Modal Usuário
    if (dom.btnAddUsuario) {
        dom.btnAddUsuario.addEventListener('click', () => {
            editingUsuarioId = null;
            document.getElementById('modal-usuario-titulo').textContent = 'Cadastrar Novo Usuário';
            document.getElementById('btn-salvar-usuario').textContent = 'Salvar Usuário';
            document.getElementById('usuario-email').disabled = false;
            dom.formUsuario.reset();
            
            // Ativa todos por padrão
            document.getElementById('user-module-dashboard').checked = true;
            document.getElementById('user-module-empresas').checked = true;
            document.getElementById('user-module-fila').checked = true;
            document.getElementById('user-module-usuarios').checked = true;
    ...
    if (dom.btnCloseModalUsuario) dom.btnCloseModalUsuario.addEventListener('click', fecharModalUsuario);
    if (dom.btnCancelarUsuario) dom.btnCancelarUsuario.addEventListener('click', fecharModalUsuario);

    // Envio Formulário Usuário
    if (dom.formUsuario) {
        dom.formUsuario.addEventListener('submit', async (e) => {
            ...
            const modulos = [];
            if (document.getElementById('user-module-dashboard').checked) modulos.push('DASHBOARD');
            if (document.getElementById('user-module-empresas').checked) modulos.push('EMPRESAS');
            if (document.getElementById('user-module-fila').checked) modulos.push('FILA');
            if (document.getElementById('user-module-usuarios').checked) modulos.push('USUARIOS');

            try {
                if (editingUsuarioId) {
                    const { error } = await supabaseClient
                        .from('usuarios')
                        .update({ nome, email, perfil, senha, modulos })
                    ...
                    const { error } = await supabaseClient
                        .from('usuarios')
                        .insert({ nome, email, perfil, senha, modulos });
                    if (error) throw error;
...
function renderizarUsuariosTable() {
    if (!dom.usuariosTbody) return;
    dom.usuariosTbody.innerHTML = '';
    ...
    usuariosData.forEach(user => {
        ...
        if (user.modulos && user.modulos.length > 0) {
            tagsHtml = user.modulos.map(mod => `<span class="module-badge badge-${mod.toLowerCase()}">${mod}</span>`).join(' ');
        }
        
        tr.innerHTML = `
            <td>${user.nome}</td>
            <td>${user.email}</td>
            <td><span class="badge-${user.perfil === 'Administrador' ? 'primary' : 'secondary'}">${user.perfil}</span></td>
            <td>${tagsHtml}</td>
            <td>
                <div class="table-actions">
                    <button class="btn-icon edit-btn" title="Editar Usuário" onclick="editarUsuario('${user.id}')">
                    ...
                    <button class="btn-icon delete-btn" title="Excluir Usuário" onclick="excluirUsuario('${user.id}')">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
                    </button>
                </div>
            </td>
        `;
        dom.usuariosTbody.appendChild(tr);
    });
}
...
window.editarUsuario = async (id) => {
    ...
    document.getElementById('user-module-dashboard').checked = user.modulos.includes('DASHBOARD');
    document.getElementById('user-module-empresas').checked = user.modulos.includes('EMPRESAS');
    document.getElementById('user-module-fila').checked = user.modulos.includes('FILA');
    document.getElementById('user-module-usuarios').checked = user.modulos.includes('USUARIOS');
...
function configurarAuth() {
        ...
        dom.formLogin.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data && data.length > 0) {
                    const user = data[0];
                    usuarioLogado = user;
                    safeStorage.setItem('HD_USER_SESSION', JSON.stringify(user));
                    ...
                    dom.appContainer.style.display = 'grid'; // Mudado de 'flex' para 'grid'
    ...
    if (dom.btnHeroAcessar) {
        dom.btnHeroAcessar.addEventListener('click', () => {
            dom.modalLogin.classList.add('active');
        });
    }
    
    if (dom.btnCloseModalLogin) {
        dom.btnCloseModalLogin.addEventListener('click', () => {
            dom.modalLogin.classList.remove('active');
        });
    }
    ...
    if (dom.btnLogout) {
        dom.btnLogout.addEventListener('click', () => {
            usuarioLogado = null;
            safeStorage.removeItem('HD_USER_SESSION');
            
            // Exibe a tela de login/hero
...
function verificarSessao() {
    const cached = safeStorage.getItem('HD_USER_SESSION');
    if (cached) {
        try {
            usuarioLogado = JSON.parse(cached);
            dom.heroSection.style.display = 'none';
            dom.appContainer.style.display = 'grid'; // Mudado de 'flex' para 'grid'
            configurarMenuNavegacao();
            return true;
    ...
    // Exibe hero section se não estiver logado
    dom.appContainer.style.display = 'none';
    dom.heroSection.style.display = 'flex';
    return false;
}
...
function configurarMenuNavegacao() {
    ...
    if (userNameElem) userNameElem.textContent = usuarioLogado.nome;
    
    const userRoleElem = document.getElementById('user-role');
    if (userRoleElem) userRoleElem.textContent = usuarioLogado.perfil;
    
    // Oculta itens de navegação se não houver permissão
    const navItems = [
        { module: 'DASHBOARD', btn: document.querySelector('.nav-btn[data-screen="dashboard-screen"]') },
        { module: 'EMPRESAS', btn: document.querySelector('.nav-btn[data-screen="empresas-screen"]') },
        { module: 'FILA', btn: document.querySelector('.nav-btn[data-screen="fila-screen"]') },
        { module: 'USUARIOS', btn: document.querySelector('.nav-btn[data-screen="usuarios-screen"]') }
    ];
    ...
    navItems.forEach(item => {
        ...
        const temPermissao = usuarioLogado.modulos && usuarioLogado.modulos.includes(item.module);
        if (temPermissao) {
            item.btn.style.display = 'flex';
            if (!primeiroPermitido) primeiroPermitido = item;
        } else {
            item.btn.style.display = 'none';
        }
    });
    
    // Direciona para a primeira tela permitida
    if (primeiroPermitido) {
        primeiroPermitido.btn.click();
    }
}
